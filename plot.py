...
def determine_plot_type(activation_function):
    ...
    if activation_function == numerical.linear:
        plot_type = "Linear"
    if activation_function == numerical.sin:
        plot_type = "Sin"
...
def bias_input_dist(input_x, Bhist, width, depth, Cw, Cb, Mu, Nboot, plot_type):
    ...
    if p < 0.05:
        ...
        print("->Input bias distribution isn't Gaussian.")
    else:
        title_add = "Gaussian"
    ...
    bias_plot_name = f"{width}w & {depth}d, Nboot = {Nboot}, Cb={round(Cb, 2)}"
    #plt.title(f"{bias_plot_name}")

    os.makedirs(f"Input Dists\{plot_type}\Bias In. Dist\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}", exist_ok=True)
    plt.savefig(f'Input Dists\{plot_type}\Bias In. Dist\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}\{width}w {depth}d.png')
    # plt.show()
    plt.clf()


#Plots the input distribution for the randomly generated weights
def weight_input_dist(input_x, Whist, width, depth, Cw, Cb, Mu, Nboot, plot_type):
    ...
    if p < 0.05:
        title_add = "Not Gaussian"
        print("->Input weight distribution isn't Gaussian")
    else:
        title_add = "Gaussian"
        print("->Input weight distribution is Gaussian")

    plt.hist(Whist, bins=100)
    plt.xlabel('Weight')
    plt.ylabel('Frequency')
    #plt.title(f'Weight Input Distribution ({title_add})')
    weight_plot_name = f"{width}w & {depth}d, Nboot = {Nboot}, Cw={round(Cw, 2)}"
    plt.title(f"{weight_plot_name}")

    os.makedirs(f"Input Dists\{plot_type}\Weight In. Dist\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}", exist_ok=True)
    plt.savefig(f"Input Dists\{plot_type}\Weight In. Dist\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}\{width}w {depth}d.png")
    # plt.show()
    plt.clf()


#Plots the final layer output distribution
def final_out_dist(output_array, input_x, width, depth, Cw, Cb, Mu, Nboot, plot_type, rescale):
    ...
    if p < 0.05:
        title_add = "Not Gaussian"
        print("->Final layer output distribution isn't Gaussian")
    if p >= 0.05:
        title_add = "Gaussian"
    ...
    plt.xlabel('Output [$z^{(l)}$]')
    plt.ylabel('Frequency')
    if rescale == False:
        plt.suptitle(f'Output Distribution ({title_add} - Not Rescaled)')
    else:
    ...
    os.makedirs(f"Output Dists\Final Layer\{plot_type}\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}", exist_ok=True)
    plt.savefig(f"Output Dists\Final Layer\{plot_type}\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}\{width}w {depth}d.png")
    #plt.show()
    plt.clf()
...
def output_dist_per_layer(A, input_x, width, depth, i, Cw, Cb, Mu, Nboot, plot_type, rescale):
    ...
    counts, bins, _, = plt.hist(A, bins=100, label="Data")
    ...
    coeff, var_matrix = curve_fit(gaussian, bins[:-1], counts, p0=p0)
    STD_fit = np.abs(coeff[1])
    STD_fit_err = np.sqrt(np.diag(var_matrix)[1])
    STD_data = std_dev(A)
    STD_data_err = STD_data/np.sqrt(Nboot)
    ...
    plt.xlabel(f'Layer {i+1} Output [z]')
    plt.ylabel('Frequency')

    #if rescale == False:
        #plt.suptitle(f'Layer {i+1} Output Distribution ({title_add} - Not Rescaled)')
    #else:
        #plt.suptitle(f'Layer {i+1} Output Distribution ({title_add})')

    plt.title(f'p = {round(p, 6)}')
    #plt.text(0.55 * xlim2, 0.82 * ylim2, f"Input Params:\n{plot_type}\n{width}w @ Layer {i+1}\nCw={Cw} & Cb={Cb}\nNboot={Nboot}", fontsize=9)
    plt.text(xlim, 0.78 * ylim2, f"Dist. Stats:\nFit Var.={round(STD_fit**2, 4)}$\pm${round(STD_fit_err**2, 4)}\nData Var.={round(STD_data_err**2, 4)}$\pm${round(STD_data**2, 4)}", fontsize=9)

    #Saves and files plot to a new or existing directory
    os.makedirs(f"Output Dists\Each Layer\{plot_type}\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}\{width}w {depth}d", exist_ok=True)
    plt.savefig(f"Output Dists\Each Layer\{plot_type}\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}\{width}w {depth}d\Layer {i+1}.png", bbox_inches="tight")
    #plt.show()
    plt.clf()

    return FWHM, STD_fit, STD_fit_err, STD_data, STD_data_err


#Plots the Full Width Half Max of the output distribution as a function of depth
def gaussian_width_and_depth(input_x, width, depth, Cw, Cb, Mu, Nboot, FWHM_array, plot_type):
    ...
    os.makedirs(f"FWHM vs Depth\{plot_type}\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}", exist_ok=True)
    plt.savefig(f"FWHM vs Depth\{plot_type}\Cb={Cb} Cw={round(Cw, 3)} Nb={Nboot}\ x = {input_x}\{width}w {depth}d")
    plt.clf()
